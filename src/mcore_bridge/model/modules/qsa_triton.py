...
if TRITON_AVAILABLE:
    ...
    def _qsa_selected_kv_backward_grouped_kernel(
        ...
        for key_start in tl.range(0, loop_end, BLOCK_K):
            ...
            if STORE_SCORES or STORE_DERIVATIVES:
                score_chunk_id = query // SCORE_CHUNK
                score_row = query - score_chunk_id * SCORE_CHUNK
                score_group = batch * SCORE_CHUNKS + score_chunk_id
    ...
    def _qsa_selected_kv_backward_tail_grouped_kernel(
        ...
        tail_offsets = tl.arange(0, BLOCK_TAIL)
        if COMPACT_ROUTE:
            # Compact metadata contains complete block IDs only.  The tail is
            # the incomplete part of the query's current causal block and is
            # therefore reconstructed from the query position rather than
            # loaded from a token-route slot.
            tail_start = ((query_position + 1) // RATIO) * RATIO
            selected = tail_start + tail_offsets
            tail_length = length % RATIO
            valid = (
                (tail_offsets < tail_length)
                & (selected >= 0)
                & (selected < seq_len_k)
            )
        else:
    ...
    def _qsa_segmented_dkv_reduce_grouped_kernel(
        ...
        for occurrence_offset in tl.range(
            ...
            occurrence_slot = encoded - row * BLOCK_TOPK
            if BATCH_ONE:
            ...
            for occurrence_offset in tl.range(
                ...
                if TABLE_SCAN:
                    ...
                    if BATCH_ONE:
                        ...
                        query_batch = query // seq_len_q
                        query = query - query_batch * seq_len_q
                        same_batch = (
                ...
                if TABLE_SCAN or GROUP_UNION:
                    ...
                    score_occurrences = occurrence[None, :]
                    block_occurrence_mask = (
    ...
    def _qsa_segmented_dkv_reduce_persistent_kernel(
        ...
        """Persistent exact owner scheduler for compact derivative A/Bs.

        The regular inverse reducer launches one CTA for every block/head-tile
        pair, including empty entries.  This diagnostic uses a fixed grid and
        walks the same work domain with a compile-time stride.  It changes no
        ownership rule: a work item still owns one physical block and one
        GQA head tile, and all route occurrences are consumed exactly once.
        """
        ...
        for work in tl.range(program, total_work, NUM_PROGRAMS):
            head_work = work % (num_kv_heads * NUM_HEAD_TILES)
            kv_head = head_work // NUM_HEAD_TILES
            head_tile = head_work - kv_head * NUM_HEAD_TILES
            key_group = work // (num_kv_heads * NUM_HEAD_TILES)
            batch = key_group // num_blocks
            block = key_group - batch * num_blocks
            segment_start = tl.load(
                segment_start_ptr + key_group).to(tl.int32)
            segment_end = tl.load(
                segment_start_ptr + key_group + 1).to(tl.int32)
            if USE_OWNER_MASK:
                owner = tl.load(
                    owner_block_mask_ptr + key_group,
                )
                active = (owner != 0) & (segment_start < segment_end)
            else:
                active = segment_start < segment_end
    ...
    def _qsa_segmented_dkv_reduce_fused_heads_kernel(
        ...
        The original diagnostic consumes compact derivative buffers.  The
        recompute variant keeps the same exact block ownership but loads the
        owner's K/V block once and recomputes QK/dP from LSE/correction.  Both
        variants process all GQA head tiles sequentially and perform one final
        dK/dV store, removing the six head-tile atomic arrivals used by the
        flattened owner.
        """
    ...
    def _qsa_segment_count_blocks_kernel(
        ...
        if COMPACT_ROUTE:
            ...
            first = block_ids * RATIO
            valid = (
                (slots < block_count)
                & (slots * RATIO + RATIO <= length)
                & (first >= 0)
                & (first < seq_len_k)
                & (first + RATIO - 1 <= query_position)
                & (block_ids >= 0)
                & (block_ids < num_blocks)
            )
        else:
...
def qsa_prepare_segmented_metadata(
    ...
    ``route_block_size == 1`` consumes the compatibility token route.  When
    ...
    if route_block_size not in {1, ratio}:
        raise ValueError(
            "QSA segmented dK/dV reduction route_block_size must be 1 or ratio")
    batch, sq, k_slots = topk_indices.shape
    if route_block_size == ratio:
        # Compact route metadata stores one complete block ID per slot.  The
        # public token count still carries the selected complete-block count
        # and the incomplete causal tail length.
        block_topk = k_slots
    else:
...
def _qsa_prepare_segmented_query_union(
    ...
    """Build a diagnostic block-group -> unique-query CSR map on the device.

    The regular owner map is ordered by physical block.  A grouped owner can
    reuse Q/grad-output only when it sees a query once for several blocks, so
    this control constructs one union row per query and records the exact
    compact-owner occurrence for every block in the group.  A repeated block
    selection for one query is deliberately rejected here; the caller then
    uses the existing exact owner path instead of silently changing duplicate
    multiplicity.

    This helper is intentionally sort/scatter-heavy and opt-in.  It is a
    feasibility probe for the data layout, not a production metadata builder.
    """

    occurrences, starts, block_topk, metadata_num_blocks = segmented_metadata[:4]
    if metadata_num_blocks != num_blocks:
        raise ValueError('QSA query-union metadata block count mismatch')
    total_occurrences = int(starts[-1].item())
    groups_per_batch = triton.cdiv(num_blocks, group_blocks)
    device = occurrences.device
    total_groups = batch * groups_per_batch
    if total_occurrences == 0:
        return (
            torch.zeros(total_groups + 1, device=device, dtype=torch.int32),
            torch.empty(0, device=device, dtype=torch.int32),
            torch.empty((0, group_blocks), device=device, dtype=torch.int32),
            groups_per_batch,
        )

    counts = starts[1:] - starts[:-1]
    block_ids = torch.repeat_interleave(
        torch.arange(batch * num_blocks, device=device, dtype=torch.int32),
        counts.to(torch.long),
    )
    occurrence_ids = torch.arange(
        total_occurrences, device=device, dtype=torch.int32)
    query_rows = occurrences.index_select(0, occurrence_ids) // block_topk
    batch_ids = block_ids // num_blocks
    local_blocks = block_ids - batch_ids * num_blocks
    group_ids = batch_ids * groups_per_batch + local_blocks // group_blocks
    local_group_blocks = local_blocks.remainder(group_blocks)
    sort_key = group_ids.to(torch.int64) * seq_len_q + query_rows.to(torch.int64)
    order = torch.argsort(sort_key)
    sorted_key = sort_key.index_select(0, order)
    sorted_group = group_ids.index_select(0, order)
    sorted_query = query_rows.index_select(0, order)
    sorted_local_blocks = local_group_blocks.index_select(0, order)
    is_new = torch.ones(total_occurrences, device=device, dtype=torch.bool)
    if total_occurrences > 1:
        is_new[1:] = sorted_key[1:] != sorted_key[:-1]
    union_id = is_new.to(torch.int64).cumsum(0) - 1
    union_queries = sorted_query[is_new].contiguous()
    union_count = int(union_queries.numel())
    group_counts = torch.bincount(
        sorted_group[is_new].to(torch.long), minlength=total_groups)
    group_starts = torch.empty(
        total_groups + 1, device=device, dtype=torch.int32)
    group_starts[0] = 0
    group_starts[1:] = group_counts.cumsum(0).to(torch.int32)

    pair_key = sorted_key * group_blocks + sorted_local_blocks.to(torch.int64)
    if total_occurrences > 1 and torch.unique(pair_key).numel() != total_occurrences:
        # Preserve exact duplicate semantics by falling back to the existing
        # owner schedule.  A future builder can carry a duplicate list and
        # sum all matching occurrence weights instead of overwriting them.
        return None
    union_occurrences = torch.full(
        (union_count, group_blocks), -1, device=device, dtype=torch.int32)
    scatter_index = (
        union_id * group_blocks + sorted_local_blocks.to(torch.int64))
    union_occurrences.view(-1).scatter_(
        0, scatter_index, order.to(torch.int32))
...
def qsa_segmented_dkv_tail(
    ...
    Complete blocks are written by the inverse-CSR owner.  Compact routes do
    not contain tail IDs, so this launch reconstructs them from the query
    position and the logical selected-token count.
    """

    route_block_size = int(route_block_size)
    if route_block_size not in {1, ratio}:
        raise ValueError(
            'QSA segmented dK/dV tail route_block_size must be 1 or ratio')
    sq, batch, num_q_heads, head_dim = query.shape
...
def qsa_segmented_dkv_reduce(
    ...
    ``MCORE_BRIDGE_QSA_SEGMENT_FUSE_HEADS_RECOMPUTE=1`` selects a compact-route
    owner that fuses all GQA head tiles into one CTA, loads the owned K/V block
    once, recomputes its probability/d-score tile from LSE/correction, and
    ...
    if use_saved_derivatives and not use_saved_scores:
        raise ValueError(
            "QSA segmented d_score reuse requires probability reuse as well")
    logical_k = (
    ...
    if compact_derivatives:
        ...
        score_chunk = int(saved_scores.shape[1])
        if score_chunk <= 0 or saved_scores.shape[0] % batch:
            raise ValueError(
                "QSA segmented dK/dV reduction saved probability chunk shape is invalid")
        if saved_scores.stride(0) >= 2**31:
            raise ValueError(
                "QSA segmented saved probability chunk stride exceeds the Triton ABI")
        score_chunks = saved_scores.shape[0] // batch
        if score_chunks * score_chunk < sq:
            raise ValueError(
                "QSA segmented dK/dV reduction saved probability workspace is too small")
    else:
        score_chunk = 1
        score_chunks = 1
    ...
    persistent_owner_requested = (
        compact_derivatives
        and os.environ.get(
            'MCORE_BRIDGE_QSA_SEGMENT_PERSISTENT_OWNER', '0') != '0'
    )
    use_block_list = (
        ...
        and (
            ...
            or os.environ.get(
                'MCORE_BRIDGE_QSA_SEGMENT_FUSE_HEADS_RECOMPUTE', '0') != '0'
        )
    )
    ...
    group_block_d = int(os.environ.get(
        'MCORE_BRIDGE_QSA_SEGMENT_GROUP_BLOCK_D', '64'))
    if group_block_d not in {32, 64, 128, 256}:
        raise ValueError(
    ...
    if persistent_owner:
        persistent_ctas = int(os.environ.get(
            'MCORE_BRIDGE_QSA_SEGMENT_PERSISTENT_CTAS', '2048'))
        if persistent_ctas <= 0:
            raise ValueError(
                'QSA persistent owner CTA count must be positive')
        persistent_ctas = min(
            persistent_ctas,
        ...
        _qsa_segmented_dkv_reduce_persistent_kernel[(persistent_ctas,)](
            *persistent_kernel_args,
            RATIO=ratio,
            GROUP_SIZE=group_size,
    ...
    if group_blocks > 1:
        ...
        if group_union:
            if hybrid_min_fanout <= 0:
                raise RuntimeError(
                    'QSA query-union owner diagnostic requires a positive hybrid fanout')
            union_metadata = _qsa_prepare_segmented_query_union(
                segmented_metadata,
                batch,
                num_blocks,
                sq,
            ...
            if union_metadata is None:
                # Duplicate block IDs in one query require multiple occurrence
                # weights.  Fall back to the exact concatenated group kernel.
                group_union = False
            else:
                group_starts, group_queries, group_occurrences, group_count = union_metadata
                group_grid = (
    ...
    kernel_args = (
        ...
        grad_value.stride(2),
        grad_value.stride(3),
        (saved_scores if use_saved_scores else query).stride(0),
        (saved_scores if use_saved_scores else query).stride(1),
        (saved_scores if use_saved_scores else query).stride(2),
        (saved_scores if use_saved_scores else query).stride(3),
        (saved_d_scores if use_saved_derivatives else query).stride(0),
        (saved_d_scores if use_saved_derivatives else query).stride(1),
        (saved_d_scores if use_saved_derivatives else query).stride(2),
        (saved_d_scores if use_saved_derivatives else query).stride(3),
    )
    kernel_options = {
        'RATIO': ratio,
        'GROUP_SIZE': group_size,
    ...
    recompute_owner_requested = (
        os.environ.get(
            'MCORE_BRIDGE_QSA_SEGMENT_FUSE_HEADS_RECOMPUTE', '0') != '0'
    )
    if recompute_owner_requested and (
            route_block_size != ratio
            or not flatten_heads
            or compact_derivatives
            or owner_slot_derivatives
            or use_saved_scores
            or table_scan):
        raise RuntimeError(
            'QSA fused recompute owner requires a compact route, segmented '
            'reduction, flattened BF16 owner, no saved score workspace, and '
            'no owner table scan')
    ...
    if (fuse_owner_heads_recompute
        ...
        raise RuntimeError(
            'QSA fused recompute owner requires BLOCK_OCC * HEAD_TILE >= 16 '
            'for the SM90 MMA reduction dimension')
    recompute_owner_maxnreg = int(os.environ.get(
        'MCORE_BRIDGE_QSA_SEGMENT_FUSE_HEADS_RECOMPUTE_MAXNREG', '0'))
    ...
    elif fuse_owner_heads_recompute or fuse_owner_heads:
        fused_score_workspace = (
            saved_scores if saved_scores is not None else query)
        fused_d_score_workspace = (
            saved_d_scores if saved_d_scores is not None else query)
        fused_kernel_args = (
            query,
            grad_output,
            ...
            grad_value.stride(2),
            grad_value.stride(3),
            fused_score_workspace.stride(0),
            fused_score_workspace.stride(2),
            fused_score_workspace.stride(3),
            fused_d_score_workspace.stride(0),
            fused_d_score_workspace.stride(2),
            fused_d_score_workspace.stride(3),
        )
        fused_owner_options = {
            'RATIO': ratio,
            'GROUP_SIZE': group_size,
...
def qsa_selected_kv_backward(
    ...
    ``MCORE_BRIDGE_QSA_DKV_REDUCTION=segmented`` enables the experimental
    block-level inverse-CSR reduction when ``selected_token_group_size`` is
    supplied; the causal tail remains on a small atomic path.  Setting
    ``MCORE_BRIDGE_QSA_SEGMENT_REUSE_DERIVATIVES=1`` additionally saves the
    ...
    ``MCORE_BRIDGE_QSA_SEGMENT_FUSE_HEADS_RECOMPUTE=1`` is an opt-in compact
    ...
    if segment_reduction_requested and selected_token_group_size is not None:
        ...
        block_topk_numerator = (
            topk_indices.shape[-1]
            if compact_route
            else topk_indices.shape[-1] - (segment_ratio - 1)
        )
        supported = (
            causal
            ...
            and (not compact_route or route_block_size == segment_ratio)
            and block_topk_numerator > 0
            and (compact_route or block_topk_numerator % segment_ratio == 0)
            and (
                (block_topk_numerator if compact_route else
        ...
        if not supported:
            raise RuntimeError(
                ...
                "power-of-two compression ratio, a matching compact route "
                "when used, and a power-of-two block_topk layout"
            )
